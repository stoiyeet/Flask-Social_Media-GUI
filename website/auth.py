...
def login():
    if request.method == 'POST':
        ...
        user1 = User.query.filter_by(usern=id).first()
        if user:
            person = user
        elif user1:
            person = user1
...
def sign_up():
    if request.method == 'POST':
        ...
        user1 = User.query.filter_by(usern=username).first()
        if user:
            ...
            new_user = User(email=email, usern=username, password=generate_password_hash(
